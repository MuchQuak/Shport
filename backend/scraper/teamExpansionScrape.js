    ...
    axios.get('https://www.espn.com/' + league + '/team/injuries/_/name/' + acro)
        .then((response) => {
            ...
            $('.Wrapper.Card__Content .ContentList__Item').each((index, element) => {
                ...
                injuryObj.position = $(element).find('.Athlete__NameDetails.ml2.clr-gray-04.di.ns9').text().trim();
                injuryObj.status = $(element).find('.TextStatus').text().trim();
                injuryObj.description = $(element).find('.pt3.clr-gray-04.n8').text().trim();
                if (injuryObj.name) { 
...
async function getTransactions(league, acro) {
    ...
    */
    axios.get('https://www.espn.com/' + league + '/team/transactions/_/name/' + acro)
        .then((response) => {
            let $ = cheerio.load(response.data);
            let transactions = [];
...
async function getTeamNews(league, acro) {
    ...
    */
    ...
    axios.get(host + '/' + league + '/team/_/name/' + acro)
        .then((response) => {
            let $ = cheerio.load(response.data);
            let teamNews = [];
            $('.ResponsiveWrapper').each((index, element) => {
                let path = $(element).find('a').attr('href');
                let title = $(element).find('.contentItem__title .Truncate.Truncate--collapsed span').text().trim();
                let timeElapsed = $(element).find('.contentItem__publicationMeta.flex.ns9.mt2.clr-gray-05 .time-elapsed').text().trim();

                if (path) {
                    newsObj = {url: host + path, title: title, timeElapsed: timeElapsed};
