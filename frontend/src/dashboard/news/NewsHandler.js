...
export function joinArticles(first, second) {
    return first.concat(second)
        .filter((v,i,a)=>a.findIndex(v2=>(v2.title.trim()===v.title.trim()))===i) // make articles unique on title
        .filter((v,i,a)=>a.findIndex(v2=>['date','description', 'publishBy'].every(k=>v2[k] ===v[k]))===i) // unique on other attributes
